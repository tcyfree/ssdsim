...
typedef struct buffer_info
{
	...
	TREE_NODE	*pTreeHeader;     				 /*for search target lsn is LRU table*///（指向平衡二叉树的根节点，从而可以访问到每一个节点）

	unsigned int max_buffer_sector;				//buffer最多能缓存的扇区数量
	unsigned int buffer_sector_count;			//已经缓存了多少个扇区
