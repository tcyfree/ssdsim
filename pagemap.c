...
struct ssd_info *pre_process_write_read(struct ssd_info *ssd)
{
	...
	ssd->tracefile2=fopen("../trace/1620_2.csv","r");//以只读方式打开trace文件，从中获取I/O请求
		...
		if ((active_block != i))
		{
			...
			for (j = 0; j < ssd->parameter->page_block; j++)
			{
				...
				else
				{
					...
					int hot_w = 0;
					//查找热读
					reply = redisCommand(ssd->redis_conn, "GET r-%d", lpn); 
					// printf("r-hot:%s %d %d\n", reply->str,reply->len, lpn);
					if (reply->len)
					{
						hot_r = 1;
					}					
					...
					// }
					//查找热写
					reply = redisCommand(ssd->redis_conn, "GET w-%d", lpn); 
					// printf("r-hot:%s %d %d\n", reply->str,reply->len, lpn);
					if (reply->len)
					{
						hot_w = 1;
