...
struct seq_write{
	unsigned int lpn;                  //这里表示该子请求的逻辑页号（该子请求的目标地址）
	unsigned long read_num;          //读次数
	unsigned long write_num;          //写次数
	unsigned long hdd_num;          //写hdd次数
	unsigned long hot_num;          //写热数据次数
...
struct entry{
	...
	int hdd_flag; //是否是hdd数据 0 默认在SSD 1 在HDD 2 SSD、HDD都有
};
...
};//该结构体记录被触发的垃圾回收请求，成员是需要进行垃圾回收的地址
